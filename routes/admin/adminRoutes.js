...
router.post('/admin/drivers/add', ensureAdmin,
...
router.post('/admin/drivers/:id/update', ensureAdmin,
  upload.fields([{ name: 'profilePic' }, { name: 'licenseCopy' }]),
  async (req, res) => {
    const {
      fullName, email, phone, gender,
    ...
    if (req.files.profilePic) {
      updateFields.profilePic = req.files.profilePic[0].path.replace('public/', '');
    }
    if (req.files.licenseCopy) {
      updateFields.licenseCopy = req.files.licenseCopy[0].path.replace('public/', '');
    }
...
router.post('/admin/addAmbulance', ensureAdmin, async (req, res) => {
  ...
  if (assignedDriver) {
    await Driver.findByIdAndUpdate(assignedDriver, {
      assignedAmbulance: savedAmbulance._id
    });
  }
...
router.post('/admin/editAmbulance/:id', ensureAdmin, async (req, res) => {
  ...
  // Remove assignedAmbulance from any previous drivers
...
router.post('/admin/deleteAmbulance/:id', ensureAdmin, async (req, res) => {
  ...
  if (ambulance) {
    // Unassign ambulance from driver (if any)
    await Driver.updateMany(
